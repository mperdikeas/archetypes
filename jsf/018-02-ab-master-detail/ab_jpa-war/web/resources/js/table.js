...
function initActions() { 
    ...
    //// focusing does not yet work as I need to find a way to
    // only focus the very first time the page is loaded and
    // rely on arrow key navigation.
    $('#parentForm\\:cashGroupTbl\\:0\\:modelrow').focus(); // we can't track the selection with the focus so let's better not have any focus at all
    //$('#parentForm\\:RowNext').hide();
    //$('#parentForm\\:RowPrev').hide();
...
function hitEnter(event) {
    if(event.keyCode==ENTER_KEY_CODE){
        var element = event.target || event.srcElement;
        var father = $(element).closest('tr');
        if (isLastChild(father, element)){
            alert('lastinput');
            return false;
        }
        else {
...
processKeyUp = function(event) {
    if (event.keyCode==F9_KEY_CODE)     {
        $("#parentForm\\:cashGroupTbl\\:BtnAdd").click();
    } else if (event.keyCode==F2_KEY_CODE)     {
        $("#parentForm\\:cancelBtn").click();
    } else if (event.keyCode==F4_KEY_CODE)     {
        $("#parentForm\\:commitBtn") .click();
    } else if (event.keyCode==F8_KEY_CODE)     {
        $("#parentForm\\:BtnDel")    .click();
...
//T0DO: this method will not work for pages with multiple tables
function markRowForError(rowIndex) {
     var elem = $('tbody tr.ui-widget-content')[rowIndex];
