...
public class RegisterEJB implements IRegisterEJB.ILocal, IRegisterEJB.IRemote{
    ...
    @Override
    public boolean register(String invId, String email, String firstname, String lastname) throws SQLException {
        ...
        try {
            ...
            {
                String pstmStr = "UPDATE invitation SET accepted = NOW() WHERE guid=?";
                pstm = conn.prepareStatement( pstmStr );
                pstm.setString   (1, invId);
                pstm.executeUpdate();
            }
            conn.commit();
            return true;
