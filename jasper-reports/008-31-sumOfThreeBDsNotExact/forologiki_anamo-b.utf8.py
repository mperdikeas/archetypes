...
STR_P1004 =STR_RERP_FIRST_NAME +' '+STR_RERP_LAST_NAME +u' ΑΦΜ :'+STR_RERP_TIN
###################################
### stoixeia dapanon          #####
###################################
...
for i in range(1,RD_RTCC.len()+1):
    for j in range(1,4):
         varname = 'STR_PCAR' + AnyToString(i)+"_"+AnyToString(j)
         ll = RD_RTCC[i][j]
         u.println("type is: %s" % AnyToString(type(ll)))
#         if isinstance(ll, str):
         if isinstance(ll, unicode):
             for c in ll:
                 u.println('**************** %d' % ord(c))
         value = AnyToString(RD_RTCC[i][j])
...
for i in range(1,RD_RTRD.len()+1):
    for j in range(1,4):
        varname22 = 'BD_PDU' + AnyToString(i)+AnyToString(j)
        if j<3:
