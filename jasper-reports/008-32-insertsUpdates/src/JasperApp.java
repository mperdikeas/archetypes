...
public class JasperApp
{
    ...
    private static Map<String, Object> prepareParameters(String paramsFile, String scriptFile) throws ClassNotFoundException, SQLException, IOException {
        ...
        System.out.println(String.format("connection's  initial autocommit status is: %b", conn.getAutoCommit()));
        conn.setAutoCommit(false);
        System.out.println(String.format("connection's eventual autocommit status is: %b", conn.getAutoCommit()));
        Map<String, Object> test = new HashMap<String, Object>();
        test.put("I_formInt", 32);
        test.put("formStr", "the value for formStr");
        test.put("STR_CCC", "ccc");
        Quad<Map<String, Object>, String, String, PyException> out = null;

        out = DBPythonExposer.prepareParameters(conn, test, script, false);
        if (out.d != null)
            System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& pyException: "+out.d);
        else
            conn.commit();
    ...
    private static void writeParameters(Map<String, Object> params, File out) throws IOException {
        ...
        for (String key : keys) {
            // sb.append(key+"  -->  "+String.valueOf(params.get(key))+" (really null: "+(params.get(key)==null?"YES":"NO")+") \n");
            sb.append(String.format("%s --> %s\n", key, params.get(key)==null?"[null value]":String.valueOf(params.get(key))));
        }
